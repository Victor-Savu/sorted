...
public export
AdditionOfPermutationsCommutes : {0 a: Type} -> {xs, ys, p: List a} -> p ~@~ (xs++ys) -> DecEq a => p ~@~ (ys++xs)
...
public export
(::) : {0 a: Type} -> {0 xs, ys: List a} -> (x: a) -> xs ~@~ ys -> DecEq a => x::xs ~@~ x::ys
